...
class BeautifierOptions:
    def __init__(self):
        ...
        self.max_preserve_newlines = 10.0
...
class Beautifier:
    ...
    def remove_indent(self):
        if len(self.output) and self.output[-1] in [
            self.indent_string,
            self.preindent_string,
        ]:
            self.output.pop()
    ...
    def get_next_token(self):
        ...
        if keep_whitespace:
            ...
            while c in self.whitespace:
                if c == "\n":
                    if (
                        self.opts.max_preserve_newlines == 0
                        or self.opts.max_preserve_newlines > self.n_newlines
                    ):
                        self.n_newlines += 1
        ...
        if c in self.wordchar:
            ...
            if (
                parser_pos != len(self.input)
                and self.input[parser_pos] in "+-"
            ...
            if (
                self.wanted_newline
                and self.last_type != "TK_OPERATOR"
                and self.last_type != "TK_EQUALS"
                and not self.flags.if_line
                and (self.opts.preserve_newlines or self.last_text != "var")
            ):
                self.append_newline()
        ...
        if c == "/":
            ...
            if self.input[parser_pos] == "*":  # peek /* .. */ comment
                ...
                if parser_pos < len(self.input):
                    while not (
                        self.input[parser_pos] == "*"
                        and parser_pos + 1 < len(self.input)
                        and self.input[parser_pos + 1] == "/"
        ...
        if (
            ...
            or (
                c == "/"
                and (
                    (self.last_type == "TK_WORD" and self.last_text in ["return", "do"])
                    or (
                        self.last_type
                        in [
                            "TK_COMMENT",
            ...
            if sep == "/":
                ...
                while (
                    parser_pos < len(self.input)
                    and self.input[parser_pos] in self.wordchar
                ):
                    resulting_string += self.input[parser_pos]
                    parser_pos += 1
            return resulting_string, "TK_STRING"
        ...
        if c == "#":
            ...
            if (
                len(self.output) == 0
                and len(self.input) > 1
                and self.input[parser_pos] == "!"
            ):
                resulting_string = c
        ...
        if (
            c == "-"
            and self.flags.in_html_comment
            and self.input[parser_pos - 1 : parser_pos + 2] == "-->"
        ):
        ...
        if c in self.punct:
            while (
                parser_pos < len(self.input)
                and c + self.input[parser_pos] in self.punct
            ):
                c += self.input[parser_pos]
                parser_pos += 1
                if parser_pos >= len(self.input):
    ...
    def handle_start_expr(self, token_text):
        ...
        if self.last_text == ";" or self.last_type == "TK_START_BLOCK":
            ...
            self.last_type in ["TK_END_EXPR", "TK_START_EXPR", "TK_END_BLOCK"]
            or self.last_text == "."
        ):
    ...
    def handle_word(self, token_text):
        ...
        if token_text == "function":
            ...
            if (
                self.just_added_newline or self.last_text == ";"
            ) and self.last_text != "{":
        ...
        if self.last_type == "TK_END_BLOCK":
            if token_text not in ["else", "catch", "finally"]:
                ...
                if self.opts.brace_style in ["expand", "end-expand"]:
                    ...
                    prefix = "SPACE"
                    self.append(" ")
        elif self.last_type == "TK_SEMICOLON" and self.flags.mode in [
            "BLOCK",
            "DO_BLOCK",
        ]:
            prefix = "NEWLINE"
        ...
        if token_text in ["else", "catch", "finally"]:
            ...
            if token_text == "function" and (
                self.last_type == "TK_START_EXPR" or self.last_text in "=,"
            ):
                ...
                if (
                    self.last_type != "TK_START_EXPR" or token_text != "var"
                ) and self.last_text != ":":
                    # no need to force newline on VAR -
                    # for (var x = 0...
                    if (
                        token_text == "if"
                        and self.last_word == "else"
                        and self.last_text != "{"
                    ):
                        self.append(" ")
                    else:
            ...
            self.is_array(self.flags.mode)
            and self.last_text == ","
            and self.last_last_text == "}"
        ):
    ...
    def handle_operator(self, token_text):
        ...
        if (
            self.flags.var_line
            and token_text == ","
            and self.is_expression(self.flags.mode)
        ):
        ...
        if token_text == ",":
            ...
            or (
                token_text in ["+", "-"]
                and self.last_type
                in ["TK_START_BLOCK", "TK_START_EXPR", "TK_EQUALS", "TK_OPERATOR"]
            )

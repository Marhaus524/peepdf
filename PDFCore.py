        ...
        try:
            for hexNumber in hexNumbers:
                self.value = self.value.replace(
                    "#" + hexNumber, chr(int(hexNumber, 16))
                )
        except:
        ...
        for i in range(len(keys)):
            ...
            if (
                ...
                if ret[0] == -1:
                    ...
                    self.addError(errorMessage)
            self.encryptedValue += (
                rawValue + " " + str(valueObject.getEncryptedValue()) + newLine
            )
        ...
        for i in range(len(keys)):
            ...
            if (
                ...
                if ret[0] == -1:
                    ...
                    self.addError(errorMessage)
            self.encryptedValue += (
                rawValue + " " + str(valueElement.getEncryptedValue()) + newLine
            )
                        ...
                        if not self.isFaultyDecoding():
                            ...
                            if (
                                ...
                                if jsErrors != []:
                                    for jsError in jsErrors:
                                        errorMessage = (
                                            "Error analysing Javascript: " + jsError
                                        )
                                        if isForceMode:
                                            self.addError(errorMessage)
                                        else:
                    ...
                    if not decrypt:
                        ...
                        if self.isEncodedStream:
                            try:
                                ...
                                errorMessage = "Error decrypting stream with " + str(
                                    algorithm
                                )
                                if isForceMode:
                                    self.addError(errorMessage)
                            ...
                            if self.filterParams == None:
                                ...
                                if i >= len(paramsArray):
                                    ...
                                    if paramsObjType == "dictionary":
                                        ...
                                        paramsDict = {}
                                ret = decodeStream(
                                    self.decodedStream, filter.getValue(), paramsDict
                                )
                                if ret[0] == -1:
                                    if i == 0 and self.rawStream != self.encodedStream:
                                        ret = decodeStream(
    ...
    def encode(self):
        ...
        if len(self.decodedStream) > 0:
            ...
            if filterType == "name":
                if self.filterParams == None:
                    ret = encodeStream(
                        self.decodedStream, self.filter.getValue(), self.filterParams
                    )
                    if ret[0] == -1:
                ...
                for i in range(len(filterElements)):
                    ...
                    if filter.getType() == "name":
                        if self.filterParams == None:
                            ret = encodeStream(
                                self.rawStream, filter.getValue(), self.filterParams
                            )
                            if ret[0] == -1:
                                errorMessage = "Encoding error: " + ret[1]
                                if isForceMode:
                            ...
                            ret = encodeStream(
                                self.rawStream, filter.getValue(), paramsDict
                            )
                            if ret[0] == -1:
                                errorMessage = "Encoding error: " + ret[1]
                                if isForceMode:
                        ...
                        if not decrypt:
                            ...
                            if self.isEncodedStream:
                                try:
                                    ...
                                    errorMessage = (
                                        "Error decrypting stream with " + str(algorithm)
                                    )
                                    if isForceMode:
                                        self.addError(errorMessage)
                                    else:
...
class PDFFile:
    ...
    def addTrailer(self, newTrailerArray):
        if (
            newTrailerArray != None
            and isinstance(newTrailerArray, list)
            and len(newTrailerArray) == 2
            and (
                newTrailerArray[0] == None or isinstance(newTrailerArray[0], PDFTrailer)
            )
            and (
    ...
    def createObjectStream(self, version=None, id=None, objectIds=[]):
        ...
        for compressedId in objectIds:
            ...
            if object == None:
                errorMessage = (
                    "Object "
                    + str(compressedId)
                    + " cannot be compressed: it does not exist"
                )
                if isForceMode:
                    self.addError(errorMessage)
                    numObjects -= 1
                ...
                if objectType == "stream":
                    ...
                    if (
                        ...
                        and object.hasElement("/U")
                        and object.hasElement("/O")
                        and object.hasElement("/R")
                    ):
                    ...
                    ret = self.body[version].setObject(
                        compressedId, object, offset, modification=True
                    )
                    if ret[0] == -1:
                        errorMessage = ret[1]
                        self.addError(ret[1])
        ...
        if self.encrypted:
            ret = computeObjectKey(
                id, 0, self.encryptionKey, self.encryptionKeyLength / 8
            )
            if ret[0] == -1:
                errorMessage = ret[1]
                self.addError(ret[1])
                    ...
                    if indirectObject != None:
                        ...
                        if object != None and not object.isCompressed():
                            ...
                            if objectType in [
                                ...
                                objectType == "stream"
                                and (
                                    object.getElement("/Type") is None
                                    or (
                                        object.getElement("/Type").getValue()
                                ...
                                if objectType in [
                                    ...
                                    if (
                                        object.getElement("/Type") != None
                                        and object.getElement("/Type").getValue()
                                        == "/EmbeddedFile"
                                    ):
                                        if revision < 5:
                                            ret = computeObjectKey(
                    ...
                    if indirectObject != None:
                        ...
                        if object != None and not object.isCompressed():
                            ...
                            if objectType in [
                                ...
                                ret = computeObjectKey(
                                    id, generationNum, self.encryptionKey, numKeyBytes
                                )
                                if ret[0] == -1:
                                    errorMessage = ret[1]
                                    self.addError(ret[1])
                ...
                for indirectObject in list(indirectObjectsDict.values()):
                    if indirectObject != None:
                        ...
                        if object != None:
                            ...
                            if (
                                re.findall(
                                    "\D" + str(id) + "\s{1,3}\d{1,3}\s{1,3}R", value
                                )
                                != []
                            ):
            ...
            for indirectObject in list(indirectObjectsDict.values()):
                if indirectObject != None:
                    ...
                    if object != None:
                        value = object.getValue()
                        if (
                            re.findall("\D" + str(id) + "\s{1,3}\d{1,3}\s{1,3}R", value)
                            != []
                        ):
                            matchedObjects.append(indirectObject.id)
        return matchedObjects
    ...
    def getStats(self):
        ...
        for version in range(self.updates + 1):
            ...
            objectsById = sorted(
                self.body[version].getObjectsIds(), key=lambda x: int(x)
            )
            ...
            if numFaultyObjects > 0:
                statsVersion["Errors"] = [
                    ...
                    self.body[version].getFaultyObjects(),
                ]
            else:
            ...
            if numStreams > 0:
                ...
                if numDecodingErrors > 0:
                    statsVersion["Decoding Errors"] = [
                        ...
                        self.body[version].getFaultyStreams(),
                    ]
                else:
                                ...
                                if numSubSectionsInXref != 0:
                                    ...
                                    if ret[0] == -1:
                                        ret = section.addEntry(
                                            id, PDFCrossRefEntry(offset, 0, "n")
                                        )
                                        if ret[0] == -1:
                                            self.addError(ret[1])
...
class PDFParser:
    ...
    def parse(self, fileName, forceMode=False, looseMode=False, manualAnalysis=False):
        ...
        matchVersion = re.findall(
            "%(PDF-|!PS-Adobe-\d{1,2}\.\d{1,2}\sPDF-)(\d{1,2}\.\d{1,2})", versionLine
        )
        if matchVersion == []:
            if forceMode:
                        ...
                        if isForceMode:
                            if pdfCrossRefSubSection != None:
                                pdfCrossRefSubSection.addError(
                                    "Bad format for cross reference entry: " + line
                                )
                            else:
                                pdfCrossRefSubSection = PDFCrossRefSubSection(
                                    0, offset=-1
    ...
    def getIndirectObjects(self, content, looseMode=False):
        ...
        if not looseMode:
            ...
            regExp = re.compile(
                "((\d{1,10}\s\d{1,10}\sobj).*?)\s\d{1,10}\s\d{1,10}\sobj", re.DOTALL
            )
    ...
    def readObject(self, content, objectType=None, forceMode=False, looseMode=False):
        ...
        for delim in delimiters:
            ...
            if content[0] == "t" or content[0] == "f":
                ...
                re.findall("^(\d{1,10}\s{1,3}\d{1,10}\s{1,3}R)", content, re.DOTALL)
                != []
            ):
                ...
                if char == delim[1] or (char + nextChar) == delim[1]:
                    ...
                    if (char + nextChar) != "<<":
                        ...
                        if ret[0] != -1:
                            ...
                            return ret
                    ret = self.readSymbol(
                        content, self.delimiters[delimIndex][1], False
                    )
                    if ret[0] != -1:

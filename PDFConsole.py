...
class PDFConsole(cmd.Cmd):
    ...
    def postloop(self):
        if self.use_rawinput:
            print(
                newLine + "Leaving the Peepdf interactive console...Bye! ;)" + newLine
            )
        self.leaving = True
    ...
    def help_bytes(self):
        ...
        print(
            newLine
            + "Shows or stores in the specified file $num_bytes of the file beginning from $offset"
            + newLine
        )
    ...
    def help_changelog(self):
        ...
        print(
            newLine
            + "Shows the changelog of the document or version of the document"
            + newLine
        )
        ...
        if elementType == "pdf":
                ...
                if pdfType == "open_action_js":
                    if numArgs > 3:
                        ...
                        if not os.path.exists(jsFile):
                            message = (
                                '*** Error: The file "' + jsFile + '" does not exist!!'
                            )
                            self.log_output("create " + argv, message)
                            return False
            ...
            if self.use_rawinput:
                res = input(
                    warning
                    + newLine
                    + "Which objects do you want to compress? (Valid respones: all | 1-5 | 1,2,5,7,8) "
                )
            else:
                res = "all"
            if res == "all":
    ...
    def help_create(self):
        ...
        print(
            newLine
            + "Creates a new simple PDF file or one with Javascript code to be executed when opening the file. It's possible to specify the file where the Javascript code is stored or do it manually."
            + newLine * 2
        )
        print("Usage: create object_stream [$version]" + newLine)
        print(
            "Creates an object stream choosing the objects to be compressed." + newLine
        )
    ...
    def help_embed(self):
        ...
        print(
            newLine
            + 'Embeds the specified file in the actual PDF file. The default type is "application/pdf".'
            + newLine
        )
        print("Options:")
    ...
    def help_encode_strings(self):
        ...
        print(
            newLine
            + "Encodes the strings and names included in the file, object or trailer"
            + newLine
        )
    ...
    def help_encrypt(self):
        ...
        print(
            newLine
            + "Encrypts the file with the default or specified password"
            + newLine
        )
    ...
    def do_extract(self, argv):
        ...
        for version in range(len(extractedJsPerObject)):
            for extractedJs in extractedJsPerObject[version]:
                output += "// peepdf comment: Javascript code located in object %d (version %d)%s%s%s" % (
                    extractedJs[0],
                    version,
                    newLine * 2,
                    extractedJs[1],
    ...
    def do_filters(self, argv):
        ...
        if filters == []:
            ...
            if newErrors != errors:
                message = (
                    "Warning: Some errors found in the modification process!!" + newLine
                )
        self.log_output("filters " + argv, message + value, [value], bytesOutput=True)

    def help_filters(self):
        print(
            newLine + "Usage: filters $object_id [$version] [$filter1 [$filter2 ...]]"
        )
        print(
            newLine
    ...
    def help_help(self):
        ...
        print(
            newLine
            + "Shows the available commands or the usage of the specified command"
            + newLine
        )
    ...
    def do_info(self, argv):
        ...
        if len(args) == 0:
            ...
            for version in range(len(statsDict["Versions"])):
                ...
                stats += (
                    beforeStaticLabel
                    + "\tObjects ("
                    + statsVersion["Objects"][0]
                    + "): "
                    + self.resetColor
                    + str(statsVersion["Objects"][1])
                    + newLine
                )
                if statsVersion["Compressed Objects"] is not None:
                    stats += (
                        beforeStaticLabel
                        + "\tCompressed objects ("
                        + statsVersion["Compressed Objects"][0]
                        + "): "
                        + self.resetColor
                        + str(statsVersion["Compressed Objects"][1])
                        + newLine
                    )
                if statsVersion["Errors"] is not None:
                    stats += (
                        beforeStaticLabel
                        + "\t\tErrors ("
                        + statsVersion["Errors"][0]
                        + "): "
                        + self.resetColor
                        + str(statsVersion["Errors"][1])
                        + newLine
                    )
                stats += (
                    beforeStaticLabel
                    + "\tStreams ("
                    + statsVersion["Streams"][0]
                    + "): "
                    + self.resetColor
                    + str(statsVersion["Streams"][1])
                )
        ...
        if id == "xref":
            ...
            if statsDict["Stream"] is not None:
                ...
                stats += (
                    beforeStaticLabel + "Stream: " + self.resetColor + "No" + newLine
                )
            ...
            if statsDict["Encrypted"]:
                ...
                stats += (
                    beforeStaticLabel + "Encrypted: " + self.resetColor + "No" + newLine
                )
            if statsDict["Errors"] is not None:
                stats += (
                    beforeStaticLabel
            ...
            stats += (
                ...
                + "References: "
                + self.resetColor
                + statsDict["References"]
                + newLine
            )
            if statsDict["JSCode"]:
                stats += (
                    beforeStaticLabel + "JSCode: " + self.resetColor + "Yes" + newLine
                )
                ...
                if statsDict["URLs"]:
                    stats += (
                        beforeStaticLabel + "URLs: " + self.resetColor + "Yes" + newLine
                    )
            if statsDict["Errors"]:
                if statsDict["Object"] == "stream":
                    stats += (
    ...
    def do_js_analyse(self, argv):
            ...
            if src not in self.variables:
                ...
                if not isJavascript(content):
                    if self.use_rawinput:
                        ...
                        print(
                            "Warning: the object may not contain Javascript code..."
                            + newLine
                        )
        elif type == "file":
            if not os.path.exists(src):
        ...
        if jsErrors != []:
            ...
            for jsError in jsErrors:
                jsanalyseOutput += (
                    "*** Error analysing Javascript: " + jsError + newLine
                )
        ...
        if object.containsJS():
            ...
            if len(jsCode) > 1:
                if self.use_rawinput:
                    res = input(
                        newLine
                        + "There are more than one Javascript code, do you want to see all (1) or just the last one (2)? "
                    )
                else:
                    res = "1"
                if res == "1":
    ...
    def help_js_eval(self):
        ...
        print(
            newLine
            + "Evaluates the Javascript code stored in the specified variable, file, object or raw code in a global context"
            + newLine
        )
    ...
    def help_metadata(self):
        ...
        print(
            newLine
            + "Shows the metadata of the document or version of the document"
            + newLine
        )
        ...
        if elementType not in validModifyTypes:
            ...
            if numArgs == 2:
                ...
                if not os.path.exists(contentFile):
                    message = (
                        '*** Error: The file "' + contentFile + '" does not exist!!'
                    )
                    self.log_output("modify " + argv, message)
                    return False
            else:
    ...
    def help_rawobject(self):
        ...
        print(
            newLine
            + "Shows the content of the object without being decoded or decrypted (object_id, xref, trailer)"
            + newLine
        )
    ...
    def help_replace(self):
        ...
        print(
            newLine + "Replaces $string1 with $string2 in the whole PDF file" + newLine
        )
        print("Usage: replace variable $var_name $string1 $string2")
        print("Usage: replace file $file_name $string1 $string2")
        print(
            newLine
            + "Replaces $string1 with $string2 in the content of the specified variable or file"
            + newLine
        )
    ...
    def help_sctest(self):
        ...
        print(
            newLine
            + "Wrapper of the sctest tool (libemu) to emulate shellcodes. With -v the output is verbose, be ready for tons of data ;p"
            + newLine
        )
    ...
    def help_tree(self):
        ...
        print(
            newLine + "Shows the tree graph of the file or specified version" + newLine
        )
    ...
    def do_vtcheck(self, argv):
        ...
        if self.variables["vt_key"][0] == "COPY_HERE_YOUR_API_KEY":
            message = (
                '*** Error: The "vt_key" variable has not been set!! You need to use your own VirusTotal API key ;)'
                + newLine * 2
                + 'Copy the key in the source code (peepdf.py:34) or define the variable "vt_key":'
                + newLine * 2
                            ...
                            if (
                                ...
                                if engineResults["detected"]:
                                    output += "%25s\t%18s\t%10s\t%s%s%s%s" % (
                                        engine,
                                        engineResults["version"],
                                        engineResults["update"],
                                        self.alertColor,
    ...
    def help_xor(self):
        ...
        print(
            newLine
            + "Performs an XOR operation using the specified key with the content of the specified file or variable, raw bytes of the file or stream/rawstream."
        )
        print(
    ...
    def do_xor_search(self, argv):
        ...
        if found:
            ...
            message = (
                "Pattern found with the following keys: " + str(keys) + newLine * 2
            )
            for key in keys:
                message += (
                    "Offsets for key '"
    ...
    def help_xor_search(self):
        ...
        print(
            "raw bytes of the file or stream/rawstream. The output shows the offset/s where the string is found. It's a case sensitive search but"
        )
    ...
    def addObject(self, iteration, maxDepth=10):
        ...
        message = (
            "What type of object do you want to include? (1-9)"
            + newLine
            + "\t1 - boolean"
            + newLine
            + "\t2 - number"
                ...
                if (
                    ...
                    while len(lines) > 0:
                        ...
                        if len(lines) == 0:
                            break
                        ch = input(
                            "( Press <intro> to continue or <q><intro> to quit )"
                        )
                        if ch == "q" or ch == "Q":
                            break
            ...
            if contentFile is not None and iteration == 0:
                ...
                if iteration == 0:
                    content = input(
                        ...
                        + "Please, specify the "
                        + newObjectType
                        + " object content (if the content includes EOL characters use a file instead):"
                        + newLine * 2
                    )
                else:
                    value = object.getValue()
        ...
        if bytes != "":
            ...
            if i is not None and i % row != 0:
                if hexChain == "":
                    ...
                    output += (
                        hexChain + (48 - len(hexChain)) * " " + "  |" + strings + "|"
                    )
        return output
    ...
    def printTreeNode(self, node, nodesInfo, expandedNodes=[], depth=0, recursive=True):
        ...
        if node in nodesInfo:
            if node not in expandedNodes or (node in expandedNodes and depth > 0):
                output += (
                    "\t" * depth + nodesInfo[node][0] + " (" + str(node) + ")" + newLine
                )
